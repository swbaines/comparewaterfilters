          ...
          {(() => {
            ...
            const rows: { label: string; render: (rec: Recommendation) => React.ReactNode }[] = [
              ...
              { label: "Coverage", render: (r) => r.idealHomeType },
              {
                ...
                render: (r) => {
                  const chlorineIds = ["under-sink-carbon", "whole-house", "whole-house-combo"];
                  return chlorineIds.includes(r.id)
                    ? <Check className="h-4 w-4 text-primary" />
                    : r.id === "under-sink-ro" ? <Check className="h-4 w-4 text-primary" /> : <span className="text-muted-foreground">—</span>;
                },
              },
              {
