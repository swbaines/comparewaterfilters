...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch live lead prices (admin-editable in lead_prices table — applied immediately)
    const { data: priceRows } = await supabaseAdmin
      .from("lead_prices")
      .select("system_type, price_per_lead");

    const resolvePrice = (type: string, fallback: number): number => {
      const row = (priceRows || []).find((p: any) => p.system_type === type);
      return row ? Number(row.price_per_lead) : fallback;
    };

    const ownerLeadPrice = resolvePrice("owner_lead", DEFAULT_OWNER_LEAD_PRICE);
    const rentalLeadPrice = resolvePrice("rental_lead", DEFAULT_RENTAL_LEAD_PRICE);
