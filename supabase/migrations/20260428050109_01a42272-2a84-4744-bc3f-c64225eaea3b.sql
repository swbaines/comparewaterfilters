...
-- 1. Update get_matched_vendors: require provider to supply ALL recommended systems
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE p.approval_status = 'approved'
      ...
      -- Strict AND: provider must supply ALL recommended systems
...
-- 2. Strip 'hybrid' from existing providers
UPDATE public.providers
SET system_types = array_remove(system_types, 'hybrid')
WHERE 'hybrid' = ANY(system_types);

-- 3. Remove 'hybrid' from valid system_type_ids registry (only if no longer referenced)
DELETE FROM public.system_type_ids WHERE id = 'hybrid';
