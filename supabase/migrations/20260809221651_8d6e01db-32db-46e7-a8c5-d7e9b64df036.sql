CREATE OR REPLACE FUNCTION public.get_matched_vendors(_customer_lat numeric, _customer_lng numeric, _customer_state text, _recommended_systems text[], _limit integer DEFAULT 10, _is_rental boolean DEFAULT false)
...
DECLARE
  _state_tokens text[];
BEGIN
  _state_tokens := ARRAY[_customer_state] || CASE upper(coalesce(_customer_state,''))
    ...
    WHEN 'NT'  THEN ARRAY['METRO_DAR','METRO_DRW']
    ELSE ARRAY[]::text[]
  END;
  ...
  candidates AS (
    ...
    WHERE p.approval_status = 'approved'
      ...
      AND (_is_rental = false OR p.accepts_rental_leads = true)
      AND (
        p.states && _state_tokens
        OR (p.service_base_state IS NOT NULL AND p.service_base_state = _customer_state)
      )
  )
  SELECT
    ...
    c.service_base_suburb, c.service_base_state, c.service_radius_km, c.distance_km,
    vp.avg_resp_min,
    vp.wr,
    CASE
